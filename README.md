...
- Full live-test encoded/decoded artifacts also remain canonical; the legislative window carries their outcome receipt (token delta, fidelity, grader loss, and bounded audit details) instead of duplicating both payloads. The structured schema describes the same substantive-deliberation boundary enforced locally.
