...
existing tasks are unmet, incomplete, or only partially satisfied, update the Spec Kit
artifact ledger when acceptance narrowed to a beta/staging/proxy/dry-run path, and
**append each piece of remaining work as a new, traceable task** at the bottom of
`tasks.md` so that `__SPECKIT_COMMAND_IMPLEMENT__` can complete it. This
command MUST run only after `__SPECKIT_COMMAND_IMPLEMENT__` has run on the
current `tasks.md`, and after `__SPECKIT_COMMAND_TASKS__` has produced a
complete `tasks.md`.
...
**BOUNDED ARTIFACT WRITES, NEVER REWRITE**: The command's only allowed writes are:

- update only the `Implementation State Ledger` section in `spec.md`;
- append a new `## Phase N: Convergence` section to `tasks.md`;

It MUST NOT:

- modify any other `spec.md` section or modify `plan.md` in any way;
...
When the codebase already satisfies everything and no artifact ledger update is needed,
the command MUST leave `spec.md` and `tasks.md` **byte-for-byte unchanged** and report a
clean result.
...
- Convergence phases and remaining contract gap tasks
...
- **Requirements inventory**: one stable key per FR-### / SC-### / user-story acceptance
  ...
  impose buildable obligations, including documentation-context constraints that
  affect implementation correctness.
...
- **`doc-drift`**: the code, plan state, or tasks depend on behavior that
  contradicts a recorded source-of-truth or official/current documentation
  reference.
- **`request-drift`**: implemented or requested work cannot be traced to an
  approved Spec Kit task, gate, acceptance check, or planned stop.
- **`ledger-drift`**: evidence or implementation state shows a beta, staging,
  proxy, dry-run, or alternate transport pass before production, but `spec.md`
  and `tasks.md` do not record the current pass, production planned stop, and
...
- **Little or no code yet**: treat the entire specified scope as `missing` remaining work
  rather than failing.
- **Nothing remains**: produce zero findings. If the artifact ledger is current,
  follow the converged branch in Step 7; if only the artifact ledger is stale,
  follow the ledger_updated branch.

### 5. Assign Severity

- **CRITICAL**: violates a constitution MUST principle, or a `missing`,
  `contradicts`, `doc-drift`, `ledger-drift`, or live/external `request-drift`
  gap that blocks baseline functionality or safe implementation of a P1 user
  story.
...
- Findings by gap type (missing / partial / contradicts / doc-drift /
  request-drift / ledger-drift / unrequested)
- Findings by severity

### 7. Update Spec Ledger And Append Convergence Tasks (or report converged)

**If there are one or more actionable findings** (`tasks_appended` outcome):

First, if a beta, staging, proxy, dry-run, or alternate transport path is the current
implementation pass while the official/production path remains incomplete or blocked,
update only the `Implementation State Ledger` section in `spec.md` with:

- current implementation pass;
- official/production path state as `planned_stop` or `blocked`;
- remaining contract gaps;
- evidence artifact references.

Then append to the **end** of `tasks.md`, per the append contract:
...
3. Emit one checklist item per actionable finding, ordered CRITICAL/HIGH first, assigning
   ...
   `<gap-type>` is one of `missing`, `partial`, `contradicts`, `doc-drift`,
   `request-drift`, `ledger-drift`, `unrequested`.
...
**If there are no actionable findings but a spec ledger update is needed**
(`ledger_updated` outcome):

- Update only the `Implementation State Ledger` section in `spec.md`.
- Do **not** append an empty convergence phase to `tasks.md`.
- Report the current implementation pass, the production planned stop, and
  whether any remaining contract gaps already have tasks.

**If there are no actionable findings and no spec ledger update is needed** (`converged` outcome):

- Do **not** modify `spec.md` or `tasks.md` at all — no empty phase header.
...
- On `ledger_updated`: state that no new tasks were needed, name the ledger
  fields updated in `spec.md`, and recommend review / PR only if production
  planned stops are acceptable for the current contract.
...
- For each remaining hook, do **not** attempt to interpret or evaluate hook `condition` expressions:
  - If the hook has no `condition` field, or it is null/empty, treat the hook as executable
  - If the hook defines a non-empty `condition`, skip the hook and leave condition evaluation to the HookExecutor implementation
- Report the convergence outcome (`converged`, `ledger_updated`, or
  `tasks_appended`) in-session before listing any hooks, so users can decide
  whether to run optional follow-up commands.
- For each executable hook, output the following based on its `optional` flag:
  - **Optional hook** (`optional: true`):
