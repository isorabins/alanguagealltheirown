...
| G18 Legislative event-replay removal | Under the exact 2026-07-30 `$1` recovery approval, prove the full production context with and without event replay, remove only transient event replay, add a mandatory public-output example, publish through the normal PR path, and observe the next normal B turn | Three consecutive production-shaped Kimi calls validate before deploy; one live B action validates; complete legislature/current state/collaboration remain model-facing; canonical events/rules/research stay full; timer returns active | turn-1213 receipt, diagnostic matrix, regression suite, fixed-point review, PR/merge/VPS hashes, live B receipt, provider/service/public-route readbacks | any diagnostic invalidity, canonical hash/record drift, model/routing/schema/validator/retry/cadence change, unauthorized rule delta, or service/repository warning | keep timer paused; preserve the turn-1213 state/cost ledger; repair only the transient prompt through the approved PR loop |
