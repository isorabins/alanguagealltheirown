...
- A stable request/idempotency id is persisted before side effects and sent in both places. Synchronous success requires a terminal `results.x.success` plus a URL/id; queued/processing results stay pending. Background work is polled by `request_id`; ambiguous timeouts poll before any retry.
