...
- Baseline: `origin/main` at `96e681f`
...
1. **Authority bypasses:** search for every rule mutation and confirm `apply_authorized_motion` is the sole live legislature writer. Try B against an older proposal, A votes, Unicode ids, nested verbs, multiple motions, repeats, and settled rules.
...
5. **Queue races:** expire and re-claim a lease, let the stale owner ack, replay ids after restart, reorder moderation/suggestion records, and verify only the current owner can remove the queue head.
...
10. **Operational truth:** verify state files are unchanged, no paid/live call occurred, original dirty checkout remains untouched, and documentation never describes unrun production behavior as live.
...
git diff --check
git diff --name-only -- state viewer/state.js
