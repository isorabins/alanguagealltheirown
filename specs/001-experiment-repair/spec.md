...
- **FR-069**: Model-context compaction MUST be a deterministic prompt-only projection. Full canonical rulebook, event, collaboration, action, current-state, and post-state receipt models and persisted JSON MUST remain unchanged.
- **FR-070**: A model-window post-state receipt MUST contain only turn, actor, result/reason, attempts, changed rule ids, current open motion, adopted count/hash, rulebook version/hash, and next actor. It MUST omit the duplicated attempted action, unchanged rule ids, and rulebook change counter from the prompt projection only.
- **FR-071**: The prompt-specific current-state projection MUST retain its authority/protocol marker, turn role, next test, next actor, rulebook version/change counter/hash, open motion, and adopted count/hash, but MUST omit the duplicate complete rule-status list because `COMPLETE LEGISLATURE` remains the single full model-facing rule/status view.
...
- **FR-075**: Live-test events and their full encoded/decoded artifacts MUST remain canonical and publicly inspectable, but MUST NOT be replayed into a fresh legislative model request. The authoritative current state/latest receipt, complete legislature, and bounded collaboration input MUST be the only historical/state inputs to that request.
...
- **FR-077**: The legislative system prompt MUST lead with a deterministic public-output contract and one non-operative example requiring a complete `Public audit:` deliberation sentence. Before deployment, the exact production-shaped prompt without event replay MUST produce three consecutive locally valid Kimi actions at the unchanged production model, routing, schema, validator, retries, and temperature.
...
- Rejected-rule tombstones, semantic/LLM summarization, prompt caching, a general workflow engine, a new database, and a generalized agent framework remain deferred. Phase 25 permits only the deterministic model-facing projections in FR-069–074.
...
- **Legislative Prompt Assembly**: An ephemeral model-facing projection of the complete canonical request, recent receipts, and one eligible collaboration delivery; it is never persisted as canonical state.
...
- **SC-037**: After the fail-closed turn-1210/1211/1213 canaries, the production-shaped B/open-`rule-132` request contains the unchanged schema and complete legislature but no event replay, remains below 55,000 characters, preserves every canonical artifact, and produces three consecutive valid provider diagnostics.
- **SC-038**: The next normal live B turn produces one valid structured action/receipt, delivers `lookup-1202-b` exactly once without truncating its canonical record, stays inside the approved `$1` recovery envelope, leaves the service healthy, and returns the normal timer to active cadence.
...
- Human-App Acceptance is also not applicable to deterministic model-context compaction because Phase 25 changes no public or interactive surface. Its offline oracle is production-shaped prompt assembly plus canonical hash/record preservation; live acceptance remains a separately managed VPS/service/provider receipt.
...
| 2026-07-30 | Phase 25 deterministic model-context compaction approved from the manager-supplied turn-1210 rehearsal: 142,488 total chars, 59,798 recent-window chars, 39,423 complete-legislature chars, 32,767 authoritative-request chars, and 14,368 pending-lookup chars | Clean worktree `codex/alato-model-context-compaction` starts at `origin/main` `9f21b0e`; production is separately paused/snapshotted by the manager and is not accessed from offline implementation | Complete T195–T201 offline, then T202 remains the manager-owned reviewed release/live B-turn acceptance gate. |
| 2026-07-30 | Phase 25 offline PASS after fixed-point remediation: the 127-rule/30-event/14,048-character lookup fixture shrank from 142,844 to 76,038 model-facing characters (46.77%); focused 54, full Python 124, JavaScript 31, and 110-requirement/202-task coverage passed with exact canonical fixture preservation, exact correlation identity, escaped-size fitting, fail-before-mutation delivery, and bounded redelivery | Reviewed code head is `a70488a601232e7c147fa2dc2c24f0a5b6ed3280`; the branch remains unpushed with no PR, and production remains paused/snapshotted at turn 1209. Review findings and the documentation-only release checkpoint are recorded in `evidence/model-context-compaction/review-and-release.md` | T202 remains open for the normal PR release, VPS sync, at-most-`$0.10` live validation, and one natural B/open-`rule-132` receipt. |
