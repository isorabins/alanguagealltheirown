...
- [ ] T007 Commit only the approved planning and preservation artifacts on `codex/experiment-repair` and record the commit in `specs/001-experiment-repair/evidence/progress.md`
...
- [ ] T008 [P] Create mixed-status, malformed-judge, collaboration, timeout, cap, and X response fixtures in `tests/fixtures/`
- [ ] T009 [P] Create a deterministic local HTTP stub for OpenRouter, Upstash, and Upload-Post responses in `tests/support/stub_server.py`
- [ ] T010 Write failing atomic JSON write/restart tests in `tests/python/test_state_store.py`
- [ ] T011 Implement temp-write/fsync/rename canonical JSON persistence and immutable snapshot hashing in `state_store.py`
- [ ] T012 [P] Create cross-artifact FR/SC/task coverage validation in `tests/acceptance/check_contract_coverage.py`
- [ ] T013 Run the foundational tests, inspect the diff, update `specs/001-experiment-repair/evidence/progress.md`, and commit the scoped foundation checkpoint
...
- [ ] T014 [P] [US1] Write failing adopted/language/legislature/proposal-trial view tests for FR-001–004 and SC-001 in `tests/python/test_rulebook_views.py`
- [ ] T015 [P] [US1] Write failing corpus-vs-proposal evidence tests for FR-003–005 in `tests/python/test_exam_evidence.py`
- [ ] T016 [P] [US1] Write failing complete/missing/duplicate/nonnumeric/out-of-range judge tests for FR-015 and SC-005 in `tests/python/test_judge_validation.py`
- [ ] T017 [P] [US1] Write equivalent Vercel adopted-only and judge contract tests in `tests/js/rulebook-and-judge.test.js`
- [ ] T018 [US1] Implement deterministic legislature, language, and single-proposal trial views plus version/hash generation in `rulebook.py`
- [ ] T019 [US1] Implement exact answer-key coverage validation returning invalid/no-score on any malformed coverage in `rulebook.py`
- [ ] T020 [US1] Replace ordinary loop encoder/decoder rulebook construction with the adopted language view and write corpus-level evidence only in `loop.py`
- [ ] T021 [US1] Preserve legacy per-rule scores/history as labeled historical data while preventing new corpus score stamping in `loop.py` and `viewer/index.html`
- [ ] T022 [US1] Port the same adopted-only version/hash and judge validator contract to `viewer/api/_lib.js` and `viewer/api/judge.js`
- [ ] T023 [US1] Remove the active dumb-script import, prompt/economics/control event path, and `probe.py` run hook while retaining historical files/records in `loop.py`, `run_turn.sh`, and `viewer/index.html` (FR-006; SC-002)
- [ ] T024 [US1] Run US1 tests and active-reference searches, attach captured prompt/state receipts to `specs/001-experiment-repair/evidence/us1-trust.md`, and verify the last-ten passing-exam calculation is unchanged (FR-038; SC-015)
- [ ] T025 [US1] Inspect and commit the scoped US1 checkpoint with no production state files modified
...
- [ ] T026 [P] [US2] Write failing A-vote, B-unrelated-proposal, repeated-settled, malformed-id, no-op, and multi-motion tests for FR-007–009 and SC-003 in `tests/python/test_motion_authority.py`
- [ ] T027 [P] [US2] Write prompt contract tests for the shared constitution, 50% target, affordability mission, and forbidden framing in `tests/python/test_prompt_contract.py`
- [ ] T028 [US2] Create the concise shared stranger/measurement/history/50%-target/access constitution with no dumb-script, power-grid, novelty, or traffic-growth claims in `prompts/constitution.md` (FR-010–011)
- [ ] T029 [US2] Rewrite the inventor contract to one propose/revise/measure action and no votes in `prompts/agent_a.md`
- [ ] T030 [US2] Rewrite the Kimi auditor contract to audit A's focused idea, vote/request focused work, and never originate unrelated rules in `prompts/agent_b.md`
- [ ] T031 [US2] Set `MODEL_B` to verified `moonshotai/kimi-k2.6` while retaining the separate stateless Kimi decoder in `loop.py`
- [ ] T032 [US2] Implement role/state/one-motion authority validation and reason-coded no-op receipts in `rulebook.py` and integrate it into `loop.py`
- [ ] T033 [US2] Render accepted and rejected legislative action honestly without inflating revision counts in `viewer/index.html`
- [ ] T034 [US2] Run US2 tests, inspect representative transcripts/no-op state diffs, attach `specs/001-experiment-repair/evidence/us2-roles.md`, and commit the scoped checkpoint
...
- [ ] T035 [P] [US3] Write failing snapshot/hash/source-mismatch/no-auto-apply tests for FR-012–014 and SC-004 in `tests/python/test_cleanup_rulebook.py`
- [ ] T036 [P] [US3] Create cleanup fixtures containing duplicate, contradictory, incomplete, historical, and disguised-operational rules in `tests/fixtures/cleanup/`
- [ ] T037 [US3] Create the bounded DeepSeek cleanup contract in `prompts/cleanup_a.md`
- [ ] T038 [US3] Create the bounded Kimi audit contract in `prompts/cleanup_b.md`
- [ ] T039 [US3] Implement snapshot-only cleanup bundle generation with explicit source/output paths and no production default in `cleanup_rulebook.py`
- [ ] T040 [US3] Implement exact diff/manifest hashes and reject omitted adopted meaning or operational text in `cleanup_rulebook.py`
- [ ] T041 [US3] Implement a separate apply subcommand requiring matching source/replacement hashes and an approval-receipt file in `cleanup_rulebook.py`
- [ ] T042 [US3] Run the cleanup suite only with stubbed model outputs, prove production files unchanged, and attach `specs/001-experiment-repair/evidence/us3-cleanup-offline.md`
- [ ] T043 [US3] Inspect and commit the scoped cleanup checkpoint without running paid calls or pausing the loop
...
- [ ] T044 [P] [US4] Write failing adopted-only, six-message alternation, judgment, persistence, and no-ledger/no-average-mutation tests for FR-025–026 and SC-008 in `tests/python/test_conversation_exam.py`
- [ ] T045 [P] [US4] Create deterministic real-work scenario and judgment fixtures in `tests/fixtures/conversation/`
- [ ] T046 [US4] Create speaker and concrete-outcome judge contracts in `prompts/conversation.md` and `prompts/conversation_judge.md`
- [ ] T047 [US4] Implement the captured-snapshot six-message artifact and requirement-level judgment in `conversation_exam.py`
- [ ] T048 [US4] Schedule Conversation once per 32 completed ordinary exams without changing ordinary exam cadence/average in `loop.py`
- [ ] T049 [US4] Publish full Conversation artifacts and visible concrete judgments in `viewer/state.js` generation and `viewer/index.html`
- [ ] T050 [US4] Run US4 tests including restart/persistence and attach `specs/001-experiment-repair/evidence/us4-conversation.md`
- [ ] T051 [US4] Inspect and commit the scoped Conversation checkpoint without real provider calls
...
- [ ] T052 [P] [US5] Write failing Redis REST enqueue/claim/lease/ack/idempotency/restart/concurrency tests in `tests/python/test_collaboration_inbox.py`
- [ ] T053 [P] [US5] Write failing Vercel inbox/session/private-access/idempotency contract tests in `tests/js/collaboration-api.test.js`
- [ ] T054 [P] [US5] Write failing RESEARCH oldest-one-per-turn, cited/no-evidence, requester-only, restart, and no-rule-mutation tests for FR-016–018 and SC-006 in `tests/python/test_research_lifecycle.py`
- [ ] T055 [P] [US5] Write failing ASK unanswered/duplicate/closed-id/verbatim/exact-once/restart tests for FR-019–021 and SC-006 in `tests/python/test_ask_lifecycle.py`
- [ ] T056 [P] [US5] Write failing wrong-password, cookie, refresh, browser-restart, 30-minute non-sliding expiry, logout, and private-queue tests for FR-020 in `tests/js/human-session.test.js`
- [ ] T057 [US5] Implement namespaced Upstash REST atomic enqueue/claim/lease/ack/session primitives in `collaboration.py`
- [ ] T058 [US5] Implement canonical `state/collaboration.json` schema, processed-id dedupe, safe reconciliation, and public sanitization in `collaboration.py` and `loop.py`
- [ ] T059 [US5] Implement the matching server-only Vercel Redis/session client with no browser token exposure in `viewer/api/_collaboration.js`
- [ ] T060 [US5] Create the bounded untrusted-evidence web-search contract in `prompts/research.md`
- [ ] T061 [US5] Parse, persist, answer at most one oldest RESEARCH request, store citations/limitations/errors, and deliver original question+result to its requester only in `loop.py`
- [ ] T062 [US5] Parse and persist ASK without blocking or auto-answering, reconcile human answers, and deliver original question+verbatim answer exactly once in `loop.py`
- [ ] T063 [US5] Implement login/session-check/logout with opaque Redis sessions and secure persistent cookies in `viewer/api/human-session.js`
- [ ] T064 [US5] Implement authenticated private ASK/suggestion reads plus read-only pending cleanup bundle/diff reads in `viewer/api/human-inbox.js`
- [ ] T065 [US5] Implement idempotent ASK answer and moderation command validation in `viewer/api/human-action.js`
- [ ] T066 [US5] Build the login, private inbox, read-only cleanup review/diff, answer, refresh, 30-minute expiry, and logout journeys without accounts/OAuth in `viewer/human.html` and route `/human` in `viewer/vercel.json`
- [ ] T067 [US5] Render public ASK awaiting/answered/delivered states and RESEARCH citations/limitations from canonical state in `viewer/index.html`
- [ ] T068 [US5] Run US5 tests and hostile text/private-access checks, attach `specs/001-experiment-repair/evidence/us5-research-ask.md`, inspect, and commit the scoped checkpoint
...
- [ ] T069 [P] [US6] Write failing submit/duplicate/approve/dismiss/restart/optional-once/private-output tests for FR-022–024 and SC-007 in `tests/js/suggestion-api.test.js` and `tests/python/test_suggestion_lifecycle.py`
- [ ] T070 [P] [US6] Add HTML/script, prompt-injection, secret-like text, rapid repeat, and oversized suggestion fixtures in `tests/fixtures/suggestions/`
- [ ] T071 [US6] Implement length-bounded idempotent public suggestion enqueue with inert text handling in `viewer/api/suggestion.js`
- [ ] T072 [US6] Place the suggestion form directly beneath the agent windows with honest pending-review copy in `viewer/index.html`
- [ ] T073 [US6] Add approve/dismiss actions and durable status feedback to the private moderation UI in `viewer/human.html`
- [ ] T074 [US6] Reconcile only approved suggestions into canonical state and keep pending/dismissed text private in `collaboration.py` and `loop.py`
- [ ] T075 [US6] Deliver at most one approved suggestion as delimited optional context outside motion parsing and record action/non-action outcome in `loop.py`
- [ ] T076 [US6] Render only approved/delivered/outcome suggestion lifecycle records publicly in `viewer/index.html`
- [ ] T077 [US6] Run restart/concurrency/hostile-output/private-surface tests and attach `specs/001-experiment-repair/evidence/us6-suggestions.md`
- [ ] T078 [US6] Inspect and commit the scoped suggestion checkpoint
...
- [ ] T079 [P] [US7] Write failing same-version/mismatch/cap/provider-error/cold-start tests for FR-027–029 and SC-009 in `tests/js/try-it.test.js`
- [ ] T080 [P] [US7] Write failing configuration tests that reject private-key fallback and require the separate public key name in `tests/js/public-key-boundary.test.js`
- [ ] T081 [US7] Make encode use the adopted language view and return rulebook version/hash without client rule text in `viewer/api/encode.js`
- [ ] T082 [US7] Make decode refetch canonical state and return `409 rulebook_changed` before a model call on version/hash mismatch in `viewer/api/decode.js`
- [ ] T083 [US7] Route all Try It provider calls exclusively through `OPENROUTER_PUBLIC_API_KEY` in `viewer/api/_lib.js`
- [ ] T084 [US7] Classify verified key-limit exhaustion as `allowance_exhausted` and all unrelated provider/network/auth failures separately in `viewer/api/_lib.js`, `encode.js`, `decode.js`, and `judge.js`
- [ ] T085 [US7] Update the Try It client to carry version/hash and show distinct normal, re-encode, reopening, and provider-failure states in `viewer/index.html`
- [ ] T086 [US7] Run production-shaped handler tests without paid calls and attach `specs/001-experiment-repair/evidence/us7-try-it.md`
- [ ] T087 [US7] Inspect and commit the scoped Try It checkpoint; leave real key/cap/WAF work open for G5
...
- [ ] T088 [P] [US8] Write failing dry/non-2xx/timeout-before/timeout-after/async/partial/idempotency/three-failure/later-note/length tests for FR-033–035 and SC-012 in `tests/python/test_tweet_delivery.py`
- [ ] T089 [P] [US8] Write desktop/375px selective-disclosure and collaboration layout tests for FR-030 and SC-011 in `tests/js/public-page.test.js`
- [ ] T090 [US8] Persist stable Upload-Post request/idempotency ids before attempts and confirm synchronous/asynchronous X receipts before posted state in `tweet.py`
- [ ] T091 [US8] Enforce `x_title`, X-only platform, copy length at most 250, no implicit thread fields, three attempts then blocked, visible blocked count, and later-note continuation in `tweet.py`
- [ ] T092 [US8] Ensure dry runs/failures do not move note watermarks or successful-post budget in `tweet.py`
- [ ] T093 [US8] Keep newest/decision-relevant agent, exam, ASK, suggestion, Conversation, and X status open while collapsing repetitive history/reference sections in `viewer/index.html`
- [ ] T094 [US8] Remove stale active dumb-script, role, experiment-status, and unsupported novelty/power/growth claims while preserving labeled history in `viewer/index.html`
- [ ] T095 [US8] Prepare but do not publish the exact field-note correction draft and original-reference receipt in `specs/001-experiment-repair/evidence/x-correction-draft.md` (FR-032; SC-014)
- [ ] T096 [US8] Prepare but do not publish one <=250-character X explainer and pin target in `specs/001-experiment-repair/evidence/x-explainer-draft.md` (FR-036; SC-014)
- [ ] T097 [US8] Research and record a small candidate follow list with primary-source reasons but perform no follows in `specs/001-experiment-repair/evidence/x-follow-draft.md` (FR-037; SC-014)
- [ ] T098 [US8] Verify the hypothetical cached economics scenario remains labeled and unchanged with no new measurement subsystem in `loop.py` and `viewer/index.html` (FR-039)
- [ ] T099 [US8] After the implemented behavior passes offline verification, update current boundaries/models/exams/collaboration/cost/deploy/interventions in `README.md` and `MECHANICS.md` (FR-031; SC-013)
- [ ] T100 [US8] Run US8 tests and stale-claim searches, attach `specs/001-experiment-repair/evidence/us8-public-record.md`, and verify Composition/Slack ASK/`:online` remain absent
- [ ] T101 [US8] Inspect and commit the scoped public-record/X checkpoint without deploy or X actions
...
- [ ] T102 Run every Python/Node/coverage test from `quickstart.md`, verify tests fail against the old baseline where meaningful, and save exact outputs in `specs/001-experiment-repair/evidence/offline-suite.md`
- [ ] T103 Inspect the full diff for secret leakage, production state changes, stale bypasses, out-of-contract files, and user-work overlap; record findings in `specs/001-experiment-repair/evidence/offline-review.md`
- [ ] T104 Re-fetch current official docs/model slugs and record any behavior drift or safe planned stop in `specs/001-experiment-repair/evidence/documentation-context.md`
- [ ] T105 Run Spec Kit convergence for the offline pass and update the Implementation State Ledger in `specs/001-experiment-repair/spec.md` plus remaining production tasks in `specs/001-experiment-repair/tasks.md`
- [ ] T106 Commit the complete offline implementation/evidence checkpoint locally and STOP for review before feature-branch push, PR, production, paid calls, credentials, or live actions
