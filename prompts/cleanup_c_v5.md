...
`assignments` maps each adopted source ID to the exact ID of a defined contract,
or to the exact reserved string `__exclude__`. For every `__exclude__` assignment,
include exactly one `exclusions` entry with the same `source_id` and an allowed
`reason`. Every exclusion entry must have that matching assignment. Do not use
alternative markers such as `excluded` or `exclude`, and do not invent a contract
to stand for an exclusion.
