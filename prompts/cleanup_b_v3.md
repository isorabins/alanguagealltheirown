...
Agent C has taken a frozen adopted rulebook and rewritten it as smaller,
strict behavior contracts plus bounded legislative memory. Your job is to determine whether Agent C achieved
...
Audit the required `trigger`, `encoder`, `decoder`, `invalid_if`, and
`overrides` fields as the operative edition. Reject missing behavior, unknown
or meaning-changing precedence, and clauses that cannot be observed. Audit the
legislative memory only as grounded continuity evidence: every entry must be
supported by its cited source IDs, must not become active law, and must not
invent a plan or duplicate an active contract.
...
- the bounded legislative memory is source-grounded and non-operative.
